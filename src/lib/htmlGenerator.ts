...
const styleToCss = (styleObj: Record<string, any>) => {
  return Object.entries(styleObj || {})
    .map(([key, val]) => {
      ...
      let cssVal = val;
      if (typeof val === "number" && !["zIndex", "opacity", "fontWeight"].includes(key)) {
        cssVal = `${val}px`;
      }
...
export const generateHTML = (elements: BuilderElement[]): string => {
  const elementsHtml = elements.map((el) => {
    ...
    const outerStyle: Record<string, any> = {
      ...
      height: isTextType ? "auto" : (typeof el.size.height === "number" ? `${el.size.height}px` : el.size.height),
      display: "flex",
      alignItems: isTextType && el.type !== "button" ? "flex-start" : "center",
    ...
    return `      <div style="${outerStyleString}">
        <div class="w-full h-full overflow-hidden flex items-center justify-center relative rounded-[inherit]">
          ${innerHtml}
        </div>
      </div>`;
  }).join("\n");
